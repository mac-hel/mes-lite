...
**Current lesson:** L15.5 - Production Readiness Review & Roadmap Closure
...
Wire build metadata into local, Docker and CI builds so the running service can identify the exact artifact version, commit and build time.

#### Business Context

Production operators need to know which build is running. When investigating an incident, rolling back a release or comparing logs and traces, `version=dev` is not enough.

#### Problem

The version package already had build-time variables, but only the short version was exposed through `/version`, and neither Docker nor CI populated those variables. The production image could run, but it could not identify the artifact that produced it.

#### Design Discussion

The project already had linker-variable placeholders, so L15.4 used Go linker flags instead of adding a configuration file, build-info dependency or generated source file. Linker flags are a common Go release technique for stamping metadata that belongs to the artifact rather than runtime configuration.

Static assets were reviewed but not expanded. The service does not currently serve custom static assets; OpenAPI is generated by Fuego at runtime and migrations are copied as runtime deployment assets. Introducing `embed` only to satisfy the roadmap would add mechanism without a business need.

#### Go Concepts

- linker flags with `-ldflags -X`
- artifact metadata versus runtime configuration
- package-level variables populated at build time
- avoiding `embed` when there are no static runtime assets to embed

#### Architecture Concepts

- release artifacts must identify themselves
- CI and Docker builds should stamp the same metadata shape
- operational endpoints should expose safe build metadata
- static asset decisions should follow runtime needs, not checklist pressure

#### Implementation

- Added `version.Info` with `version`, `commit` and `buildTime` JSON fields.
- Added `version.Current`.
- Changed `/version` to return full build metadata.
- Updated server tests for the expanded version response.
- Added version package tests.
- Added Makefile `VERSION`, `COMMIT`, `BUILD_TIME` and `LDFLAGS` variables.
- Updated `make build` to stamp build metadata.
- Added Docker build args for version, commit and build time.
- Updated the Dockerfile to stamp metadata into both server and migration binaries.
- Updated CI Docker build to pass branch/ref, commit SHA and UTC build time.
- Extended the production-image smoke script to optionally assert `/version` metadata.

#### Tests

- Verified focused version and server tests with `go test ./internal/platform/version ./internal/server -count=1`.
- Verified explicit linker-flag build with `go build -ldflags ... -o /tmp/opencode/mes-lite-version-test ./cmd/server`.
- Verified Docker image build with explicit metadata using `docker build --build-arg VERSION=smoke-version --build-arg COMMIT=smoke-commit --build-arg BUILD_TIME=2026-09-05T00:00:00Z -t mes-lite:local .`.
- Verified production-image smoke test asserts `/version` metadata with `HOST_PORT=19090 EXPECTED_VERSION=smoke-version EXPECTED_COMMIT=smoke-commit EXPECTED_BUILD_TIME=2026-09-05T00:00:00Z sh ./bin/smoke-production-image.sh`.
- Verified Makefile build metadata path with `make build VERSION=make-version COMMIT=make-commit BUILD_TIME=2026-09-05T01:02:03Z`.
- Verified with `go build ./...`.
- Verified with `go test ./... -count=1`.
- Verified with `go vet ./...`.
- Verified with `golangci-lint run ./...`.
...
The `/version` response now uses the platform version DTO directly. No business package changed.

#### Code Review

An experienced Go engineer would approve using linker flags here because build metadata belongs to the compiled artifact and should not be supplied manually as runtime configuration.

The main caveat is API compatibility: `/version` now returns more fields. This is acceptable for the project stage and the endpoint is operational rather than business-critical. Existing `version` remains unchanged, so clients that only read that field continue to work.

#### Exercises

- Build two images from different commits and compare `/version` responses.
- Change `BUILD_TIME` to a non-RFC3339 value and decide whether build metadata should be validated.
- Design when `embed` would become justified for this service.

#### Interview Questions

- What does `go build -ldflags -X` do?
- Why is build metadata different from runtime configuration?
- Why should release artifacts identify their source commit?
- When is `embed` useful, and when is it unnecessary?

#### Roadmap Update

- Lesson 15.4 completed.
- Current lesson moved to Lesson 15.5.
- Milestone 15 remains in progress.
- `embed` intentionally remains unchecked because no static runtime asset currently needs embedding.
