...
// ErrInvalidListOptions is returned when product list query options are not supported.
var ErrInvalidListOptions = errors.New("invalid product list options")

// ListOptions defines filtering, sorting and pagination for product lists.
...
func validSort(sort string) bool {
	switch sort {
	case "sku", "-sku", "name", "-name", "category", "-category":
		return true
	default:
		return false
	}
