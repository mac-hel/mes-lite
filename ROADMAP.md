...
**Current lesson:** 6.2 JWT Issuing & Verification
...
**Go knowledge progress:** 42%
**Interview readiness:** 33%
**Known technical debt:** Production reference foreign keys are `NOT VALID`, so PostgreSQL enforces new production entries but does not validate legacy rows created before the constraint. Query parameters are implemented for list endpoints; explicit OpenAPI query-parameter documentation should be reviewed later. Authentication users currently use an in-memory bootstrap store and login returns an opaque token that is not yet verified by middleware; JWT verification, protected endpoints and durable auth-user persistence remain within Milestone 6.
