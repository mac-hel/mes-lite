...
**Current lesson:** 6.5 Security Review, OpenAPI & Milestone Review
...
**Go knowledge progress:** 48%
**Interview readiness:** 40%
**Known technical debt:** Production reference foreign keys are `NOT VALID`, so PostgreSQL enforces new production entries but does not validate legacy rows created before the constraint. Query parameters are implemented for list endpoints; explicit OpenAPI query-parameter documentation should be reviewed later. Authentication users currently use an in-memory bootstrap store; durable auth-user persistence remains pending and should be reviewed before completing Milestone 6.
